...
docker_setup_env() {
	...
	_mariadb_file_env 'MYSQL_DATABASES'
...
docker_setup_db() {
	...
	if [ -n "$MARIADB_DATABASES" ]; then
		for databaseName in $MARIADB_DATABASES; do
			createDatabase="${createDatabase}CREATE DATABASE IF NOT EXISTS \`$databaseName\`;"
		done
	fi
	...
	if  [ -n "$MARIADB_PASSWORD" ] || [ -n "$MARIADB_PASSWORD_HASH" ] && [ -n "$MARIADB_USER" ]; then
		...
		if [ -n "$MARIADB_DATABASES" ]; then
			for databaseName in $MARIADB_DATABASES; do
				mysql_note "Giving user ${MARIADB_USER} access to schema ${databaseName}"
				userGrants="${userGrants}GRANT ALL ON \`${databaseName//_/\\_}\`.* TO '$MARIADB_USER'@'%';"
			done
		fi	
	fi
